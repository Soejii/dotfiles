---
name: peer-review
description: Peer review a code diff by an external model only when Suji explicitly asks to review or sanity-check a change. Never invoke this skill as an automatic pre-commit gate.
---

# Peer review on request

Run this skill only when Suji explicitly requests peer review. A reminder that review is available is allowed, but wait for his instruction before dispatching an external reviewer.
...
**Codex does not read `AGENTS.md`.** On a Flutter repo, concatenate
